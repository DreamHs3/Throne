...
| 5. Регрессии: hung Close→timeout; Start при незавершённом Close; восстановление после Close; ошибка Stop не теряется на SCM-границе | да (§3) | **да** |
...
Поправка регрессии REC-02C (2026-09-19, test hygiene, без изменения
продакшн-кода): в `TestStopWithCompletedCloseTimedOutLeavesNoRecord`
подмена `closePublishedRuntime` обслуживает ТОЛЬКО моделируемый Stop
(остальные close идут через штатную функцию), захваченный первый runtime
реально закрывается стандартным close до следующего Start, заключительный
Stop проверяется по обоим каналам (Go error и `ErrorResp`), cleanup
освобождает все созданные экземпляры в том числе при раннем падении.
Red/green перепроверены на хосте: на старом порядке регистрации тест
падает на утверждении REC-02C, с фиксом — PASS. Из строки 5 таблицы §5
удалена оставшаяся неточная формулировка «Start не публикует после
shutdown» — гарантия для уже начатого Start описана в «Точной
формулировке гарантии» (§5): краткая публикация до post-check возможна и
закрывается post-check'ом.
